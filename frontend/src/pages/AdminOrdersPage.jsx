...
import { Eye, CheckCircle, XCircle, Truck, Calendar, DollarSign, User, Loader2 } from 'lucide-react'
...
export default function AdminOrdersPage() {
  ...
  const updateStatus = async (orderId, newStatus) => {
    ...
    if(!window.confirm(`Xác nhận chuyển đơn #${orderId} sang trạng thái: ${newStatus.toUpperCase()}?`)) return;
                    ...
                    <tr className={`hover:bg-slate-700/30 transition group ${expandedOrderId === order.id ? 'bg-slate-700/20' : ''}`}>
                      ...
                      <td className="p-4">
                        <div className="font-mono text-green-400 font-bold flex items-center gap-1">
                            <DollarSign size={12}/> {new Intl.NumberFormat('vi-VN').format(order.total_price)} ₫
                        </div>
                      ...
                      <td className="p-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => setExpandedOrderId(expandedOrderId === order.id ? null : order.id)} className="p-2 bg-slate-700 text-gray-300 hover:text-white rounded-lg transition border border-slate-600" title="Xem chi tiết"><Eye size={16}/></button>
                    ...
                    {expandedOrderId === order.id && (
                        <tr className="bg-slate-900/40 animate-in fade-in zoom-in-95 duration-200">
                            <td colSpan="5" className="p-6 border-t border-slate-700/50 shadow-inner">
                                <div className="text-xs font-bold text-gray-400 uppercase mb-3 flex items-center gap-2"><Eye size={12}/> Chi tiết đơn hàng #{order.id}</div>
                                <div className="flex flex-wrap gap-4">
                                    {order.items.map((item, idx) => (
                                        <div key={idx} className="flex items-center gap-4 bg-slate-800 p-3 rounded-xl border border-slate-700 w-full sm:w-auto sm:min-w-[300px] shadow-lg">
                                            <div className="w-12 h-12 rounded-lg overflow-hidden border border-slate-600 bg-slate-900">
                                                <img src={item.image_url || 'https://via.placeholder.com/50'} alt="" className="w-full h-full object-cover"/>
                                            </div>
                                            <div>
                                                <div className="text-white font-bold text-sm">{item.product_name}</div>
                                                <div className="text-gray-400 text-xs mt-0.5">{item.variant_name} <span className="text-yellow-500 font-bold">x{item.quantity}</span></div>
                                            </div>
